...
class downsample_4x(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=4, stride=4, padding=0, dilation=1):
        super(downsample_4x, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, out_planes,kernel_size=kernel_size, stride=stride,padding=padding, dilation=dilation, bias=False)
...
class downsample_8x(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=4, stride=8, padding=0, dilation=2):
        ...
        #self.bn2 = nn.BatchNorm2d(out_planes)
        #self.relu2 = nn.ReLU(inplace=True)
        #self.conv3 = nn.Conv2d(out_planes, out_planes,kernel_size=kernel_size, stride=stride,padding=padding, dilation=dilation, bias=False)
...
class upsample_2x(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=4, stride=2, padding=1, dilation=1):
        ...
        self.conv = nn.ConvTranspose2d(in_planes, out_planes,kernel_size=kernel_size, stride=stride,padding=padding, dilation=dilation, bias=False)
        #self.bn = nn.BatchNorm2d(out_planes)
        self.relu = nn.ReLU(inplace=True)
...
class upsample_4x(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=4, stride=4, padding=0, dilation=1):
        ...
        self.conv1 = nn.ConvTranspose2d(in_planes, out_planes,kernel_size=kernel_size,
...
class upsample_8x(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=4, stride=8, padding=0, dilation=2,output_padding=1):
        ...
        self.bn1 = nn.BatchNorm2d(out_planes)
        self.relu1 = nn.ReLU(inplace=True)
        #self.conv2 = nn.ConvTranspose2d(out_planes, out_planes,kernel_size=kernel_size, stride=stride,padding=padding, dilation=dilation, bias=False)
        #self.bn2 = nn.BatchNorm2d(out_planes)
        #self.relu2 = nn.ReLU(inplace=True)
        #self.conv3 = nn.ConvTranspose2d(out_planes, out_planes,kernel_size=kernel_size, stride=stride,padding=padding, dilation=dilation, bias=False)
        #self.bn3 = nn.BatchNorm2d(out_planes)
        #self.relu3 = nn.ReLU(inplace=True)
...
class form_attention_map(nn.Module):
    def __init__(self, in_planes,kernel_size=3, stride=1, padding=1, dilation=1):
        ...
        self.conv1 = nn.Conv2d(in_planes, 32,kernel_size=kernel_size, stride=stride,padding=padding, dilation=dilation, bias=False)
        self.conv2 = nn.Conv2d(32, 1,kernel_size=1, stride=1,padding=0, dilation=dilation, bias=False)
        self.sig = nn.Sigmoid()
...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=8):
        ...
        self.fc = nn.Sequential(nn.Linear(channel, int(channel / reduction), bias=False),nn.ReLU(inplace=True),nn.Linear(int(channel / reduction), channel, bias=False))
...
class exchange(nn.Module):
    def __init__(self,scale1,scale2,scale3,scale4,k_1,k_2,attention='Original'):
        super().__init__()
        self.k_1 = 32
        self.k_2 = 64
        self.attention = attention
        
        self.layers1 = [scale1,scale1+self.k_1,scale1+2*self.k_1,scale1+3*self.k_1]
        self.layers2 = [scale2,scale2+self.k_1,scale2+2*self.k_1,scale2+3*self.k_1]
        self.layers3 = [scale3,scale3+self.k_1,scale3+2*self.k_1,scale3+3*self.k_1]
        self.layers4 = [scale4,scale4+self.k_1,scale4+2*self.k_1,scale4+3*self.k_1]
        ...
        self.v1 = BasicConv2d(self.layers3[0],self.k_1,kernel_size=3,stride=1,padding=1)
        self.w1 = BasicConv2d(self.layers4[0],self.k_1,kernel_size=3,stride=1,padding=1)
        
        self.x1_y1 = upsample_2x(self.k_1,self.k_1)
        self.x1_v1 = upsample_4x(self.k_1,self.k_1)
        self.x1_w1 = upsample_8x(self.k_1,self.k_1)
        self.att_x1 = form_attention_map(3*self.k_1)
        self.att_y1 = torch.nn.MaxPool2d(2, stride=2)
        self.att_v1 = torch.nn.MaxPool2d(2, stride=2)
        self.att_w1 = torch.nn.MaxPool2d(2, stride=2)
        ...
        self.v2 = BasicConv2d(128,self.k_1,kernel_size=3,stride=1,padding=1)
        
        self.w2 = BasicConv2d(256,self.k_1,kernel_size=3,stride=1,padding=1)
        ######### 3 calculation
        
        self.x2_y2 = upsample_2x(self.k_1,self.k_1)
        self.x2_v2 = upsample_4x(self.k_1,self.k_1)
        self.x2_w2 = upsample_8x(self.k_1,self.k_1)
        self.att_x2 = form_attention_map(3*self.k_1)
        self.att_y2 = torch.nn.MaxPool2d(2, stride=2)
        self.att_v2 = torch.nn.MaxPool2d(2, stride=2)
        self.att_w2 = torch.nn.MaxPool2d(2, stride=2)
        ...
        self.v3 = BasicConv2d(128,self.k_1,kernel_size=3,stride=1,padding=1)
        
        self.w3 = BasicConv2d(256,self.k_1,kernel_size=3,stride=1,padding=1)
        
        ################# 4 calculations
        
        self.x3_y3 = upsample_2x(self.k_1,self.k_1)
        self.x3_v3 = upsample_4x(self.k_1,self.k_1)
        self.x3_w3 = upsample_8x(self.k_1,self.k_1)
        self.att_x3 = form_attention_map(3*self.k_1)
        self.att_y3 = torch.nn.MaxPool2d(2, stride=2)
        self.att_v3 = torch.nn.MaxPool2d(2, stride=2)
        self.att_w3 = torch.nn.MaxPool2d(2, stride=2)
    ...
    def forward(self,x,y,v,w):
        ...
        if self.attention=="Reverse":
            att_x1 = 1 - att_x1
        
        x2_input = torch.cat([x,x1],axis=1)
        x2_input = self.x2_input(x2_input)
        x2_input = x2_input*att_x1
        
        att_y1 = self.att_y1(att_x1)
        
        
        y2_input = torch.cat([y,y1],axis=1)
        y2_input = self.y2_input(y2_input)
        y2_input = y2_input*att_y1
        
        att_v1 = self.att_v1(att_y1)
        
        v2_input = torch.cat([v,v1],axis=1)
        v2_input = self.v2_input(v2_input)
        v2_input = v2_input*att_v1
        
        att_w1 = self.att_w1(att_v1)
    
        w2_input = torch.cat([w,w1],axis=1)
        ...
        x3_input = torch.cat([x,x1,x2],axis=1)
        x3_input = self.x3_input(x3_input)
        x3_input = x3_input*att_x2
        
        att_y2 = self.att_y2(att_x2)
        
        y3_input = torch.cat([y,y1,y2],axis=1)
        y3_input = self.y3_input(y3_input)
        y3_input = y3_input*att_y2
        
        
        att_v2 = self.att_v2(att_y2)
        
        v3_input = torch.cat([v,v1,v2],axis=1)
        v3_input = self.v3_input(v3_input)
        v3_input = v3_input*att_v2
        
        
        att_w2 = self.att_w2(att_v2)
    
        w3_input = torch.cat([w,w1,w2],axis=1)
        ...
        x4_input = torch.cat([x,x1,x2,x3],axis=1)
        x4_input = self.x4_input(x4_input)
        x4_input = x4_input*att_x3
        
        
        att_y3 = self.att_y3(att_x3)
        
        y4_input = torch.cat([y,y1,y2,y3],axis=1)
        y4_input = self.y4_input(y4_input)
        y4_input = y4_input*att_y3
        
        
        att_v3 = self.att_v3(att_y3)
        
        v4_input = torch.cat([v,v1,v2,v3],axis=1)
        v4_input = self.v4_input(v4_input)
        v4_input = v4_input*att_v3
        
        att_w3 = self.att_w3(att_v3)
    
        w4_input = torch.cat([w,w1,w2,w3],axis=1)
        ...
        v4 = self.v4(v4_input)
        w4 = self.w4(w4_input)
        #print(x.shape,y.shape,v.shape,w.shape)
        #print(x4.shape,y4.shape,v4.shape,w4.shape)
        return x+x4,y+y4,v+v4,w+w4,att_x1,att_x2,att_x3
...
class EncoderBlock(nn.Module):
    def __init__(self):
        ...
        self.msrfv21 = exchange(32,64,128,256,32,64,attention="Original")
        self.msrfv22 = exchange(32,64,128,256,32,64)
        self.msrfv23 = exchange(32,64,128,256,32,64,attention="Reverse")
    ...
    def forward(self, x):
        ...
        n13,n23,n33,n43,att_1x_1,att_1x_2,att_1x_3 = self.msrfv21(n11,n21,n31,n41)
        #n13,n23,n33,n43 = self.msrfv22(n12,n22,n32,n42)
        n14,n24,n34,n44,att_2x_1,att_2x_2,att_2x_3 = self.msrfv23(n13,n23,n33,n43)
        ...
        return seg_map,dsv_up2,dsv_up4,dsv_up8,att_1x_1,att_1x_2,att_1x_3,att_2x_1,att_2x_2,att_2x_3
