...
def train(train_loader, model, optimizer, epoch,device):
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        trainsize = 256
            # ---- forward ----
        lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2,att_x1,att_x2,att_x3,att_2x1,att_2x2,att_2x3 = model(images)
            # ---- loss function ----
        ...
        loss1_x = structure_loss(att_x1, gts)
        loss2_x = structure_loss(att_x2, gts)
        loss3_x = structure_loss(att_x3, gts)

        loss1_x1 = structure_loss(att_2x1, gts)
        loss2_x1 = structure_loss(att_2x2, gts)
        loss3_x1 = structure_loss(att_2x3, gts)

        loss = loss2 + loss3 + loss4 + loss5 + loss1_x +loss2_x + loss3_x + loss1_x1 +loss2_x1+loss3_x1    
    ...
    if (epoch+1) % 10 == 0:
        torch.save(model.state_dict(), save_path + 'PAANet-%d.pth' % epoch)
        print('[Saving Snapshot:]', save_path + 'PAANet-%d.pth'% epoch)
...
def evaluate(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(loader):
            ...
            y_pred,_,_,_,_,_,_,_,_,_ = model(x)
